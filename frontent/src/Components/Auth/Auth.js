...
const Auth = (props) => {
    ...
    const signUp = async (e) => {
        ...
        await axios.post(`https://blood-donation-3tl1.onrender.com/auth/${handle}`, formData, { withCredentials: true }).then(async (res) => { }, (err) => alert(err.response.data.errorMessage));
        await getLoggedIn();
    ...
    const logIn = async (e) => {
        ...
        try {
            ...
            await axios.post(`https://blood-donation-3tl1.onrender.com/auth/login/${handle}`, formData, { withCredentials: true }).then(async (res) => { });
            await getLoggedIn();
            navigate(`/${handle == "bank" ? handle : "user"}/profile`)
        } catch (err) {
